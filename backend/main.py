...
app = FastAPI(
    ...
    description="Geospatial S&P 500 Risk Intelligence -- LangChain x SurrealDB x LangGraph",
...
def _startup():
    ...
    try:
        connect()
        logger.info("[OK] SurrealDB connected")
    except Exception as e:
        logger.error(f"[FAIL] SurrealDB: {type(e).__name__}: {e}")  # Fix #83
        return

    # 2. Schema
    try:
        run_schema()
        logger.info("[OK] Schema DDL")
    except Exception as e:
        logger.error(f"[FAIL] Schema: {type(e).__name__}: {e}")
    ...
    try:
        ...
        logger.info("[OK] langchain-surrealdb (VectorStore + Graph + GraphQAChain)")
    except Exception as e:
    ...
    try:
        ...
        if count < 50:
            ...
            seed_all()
            logger.info("[OK] Seed loaded")
        else:
            logger.info(f"[OK] Already seeded ({count} companies)")
    except Exception as e:
        ...
        try:
            ...
            logger.error(f"[FAIL] Seed failed: {e2}")
    ...
    try:
        ...
        logger.info("[OK] USGS poller LIVE (60s, M5.0+)")
    except Exception as e:
    ...
    try:
        ...
        logger.info("[OK] NewsAPI scanner LIVE (15min)")
    except Exception as e:
